    ...
    <div class="super">
        ...
        <div class="view-container">
            <div class="view-header">
                ...
                <a href="{{ route('document.create') }}" class="btn-upload-new">+ Upload New Quiz</a>
            </div>

            <form action="{{ route('document.selectresult') }}" method="GET" class="search-section">
                <input type="search" name="query" placeholder="Search by title..." value="">
                <button type="submit">Search</button>
                <button type="button" onclick="window.location.href='{{ route('document.selectresult') }}'">Clear</button>
            </form>

            <div class="doc-list">
                {{-- @forelse handles checking if empty and looping all at once --}}
                @forelse ($documents as $item)
                    <div class="doc-card">
                        <div class="doc-info">
                            <h3>{{ $item['filename'] }}</h3>
                            <span>Uploaded: {{ date("F j, Y, g:i a", strtotime($item['uploaded_at'])) }}</span>
                        </div>
                        <div class="actions-group">
                            {{-- Added dynamic item id parameter to the URL query --}}
                            <a class="btn-quiz" href="results/{{$item['id']}}">View Result</a>
                        </div>
                    </div>
                @empty
                    <div class="no-docs">
